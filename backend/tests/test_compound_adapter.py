"""Tests for CompoundAdapter.

Compound V3 (Comet) has no official public REST API as of 2026. The adapter
...
from app.collectors.compound import CompoundAdapter
...
async def test_fetch_reserves_raises_not_implemented() -> None:
    """CompoundAdapter raises NotImplementedError until a reliable data source is found."""
    adapter = CompoundAdapter(_ENTRY, client=AsyncMock(spec=httpx.AsyncClient))
...
async def test_fetch_reserves_raises_with_descriptive_message() -> None:
    ...
    assert "Compound" in message or "compound" in message.lower()
